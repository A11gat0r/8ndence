// Simple mock dataset (kept for client-side fallback)
const mockData = [
...
async function runDetector() {
    ...
    try {
        ...
        if (data && Array.isArray(data.results)) { renderResults(data.results); return; }
...
function closeModal() { const modal = document.getElementById('detailModal'); modal.setAttribute('aria-hidden', 'true'); }

document.addEventListener('DOMContentLoaded', () => {
    const scanBtn = document.getElementById('scanBtn'); if (scanBtn) scanBtn.addEventListener('click', runDetector);
    const navToggle = document.getElementById('navToggle'); const navList = document.getElementById('navList'); navToggle && navToggle.addEventListener('click', () => navList.classList.toggle('show'));
    const yearEl = document.getElementById('year'); if (yearEl) yearEl.textContent = new Date().getFullYear();
    const brandInput = document.getElementById('brandInput'); if (brandInput) brandInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') runDetector(); });
    const modalClose = document.getElementById('modalClose'); const modalOverlay = document.getElementById('modalOverlay'); if (modalClose) modalClose.addEventListener('click', closeModal); if (modalOverlay) modalOverlay.addEventListener('click', closeModal);
